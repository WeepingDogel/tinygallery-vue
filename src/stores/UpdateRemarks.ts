...
export const UpdateRemarks = defineStore('UpdateRemarks', { // Define the "UpdateRemarks" store module
    state: () => ({ // Define the initial state of the store
        update: 1 // Set the update status to 1 by default
    }),
    actions: { // Define the actions that can be performed on the store
        Update(status: number) { // Define an action to update the status
