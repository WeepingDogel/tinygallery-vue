...
export default {
  ...
  methods: {
      ...
      if (Token) {
        axios
          ...
          .then((response) => {
            ...
            axios
              .get("/resources/avatar/" + username, {}) // Sends a GET request to retrieve the user's avatar based on their username
              .then((response) => {
    ...
    GetTheOriginalRemark() {
      // Retrieves the original remark being replied to
      axios
        .get("/remark/get/in_remark/single/" + this.RemarkUUID) // Sends a GET request to retrieve the remark with the specified UUID
        .then((response) => {
          console.log(response.data);
    ...
    GetAllReplies() {
      // Retrieves all replies to the original remark
      axios
        .get("/remark/get/reply/" + this.RemarkUUID + "/" + this.Page) // Sends a GET request to retrieve all replies
        .then((response) => {
        ...
        axios
          ...
          .then((response) => {
            ...
            if (username != false) {
              ...
              alert(
                "Not authorized! You have to login your account to send a reply!"
              );
            }
          })
          .catch((error) => {
    ...
    <div class="ReplyPanel">
      ...
      <div class="MidControl">
        <div class="OriginalRemarkDisplay" :id="RemarkUUID">
          <img
            :src="(OriginalRemark as any).avatar"
            class="UserAvatarOfRemarkSender"
          />
          <div class="CommentText">
            <h2 class="UserNameOfRemarkSender">
              {{ (OriginalRemark as any).user_name }}
            </h2>
            <p class="OriginalRemarkContent">
              {{ (OriginalRemark as any).content }}
            </p>
            <p class="RemarkDate">
              {{
                TimeZoneCaculator.CaculateTheCorrectDate(
                  (OriginalRemark as any).date
                )
              }}
            </p>
          </div>
